...
- Bands alternate background: rows 0 and 2 are transparent; rows 1 and 3 use a soft tint of `Color(0xFF3B7A1D)` (low opacity, e.g. `withOpacity(0.12)`).
...
- Icon size: 68px.
- Text block: title in bold (fontSize 22, color `Color(0xFF3B7A1D)` on transparent rows, white on tinted rows), description below (fontSize 17, height 1.5, `Colors.black87` on transparent rows, `Colors.white70` on tinted rows). Text is left-aligned within its block.
...
|------------------|----------------------|--------------------|-----------------------------------|
| Transparent row  | `Color(0xFF3B7A1D)`  | `Colors.black87`   | transparent                       |
| Tinted row       | `Colors.white`       | `Colors.white70`   | `Color(0xFF3B7A1D).withOpacity(0.12)` |
